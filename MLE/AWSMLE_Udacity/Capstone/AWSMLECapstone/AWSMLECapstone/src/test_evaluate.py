...
def parse_inputs():
    ...
    parser.add_argument('--evaluate_test',
                        type=str,
                        default="False",
                        help="Runs only evaluation on test data (default:False(Disabled))"
                       )
    args = parser.parse_args()
    if (args.evaluate_test == "True"):
        logger.info("Attention: This is an inference job")
    else:
...
def create_data_loaders(data_path, batch_size, train=True):
    '''
...
def test_model(model, test_dl, loss_criterion, device, test, hook, hook_enabled):
    ...
    avg_test_accuracy = round(test_accuracy/len_ds, 5)
    logger.info(f'{ds_class} Loss: {round(test_loss/len_ds, 5)}')
    logger.info(f'{ds_class} Accuracy: {avg_test_accuracy}')
    ...
    return report_dict, avg_test_accuracy
...
def main(args):
    ...
    if (args.evaluate_test=="True"):
        logger.info("Evaluate Test is set to True. We will be creating only test dataloader")
    train_dl, valid_dl, test_dl = create_data_loaders(data_path, batch_size, train=False)
    ...
    if (args.evaluate_test=="True"):
        logger.info("Step 5: Test Evaluate is set to True. We will only be inferencing on model")
    else:
    ...
    test = True
    report_dict, test_accuracy = test_model(model, test_dl, loss_criterion, device, test, hook, hook_config)
    ...
    '''
    if (args.evaluate_test=="True"):
        logger.info("Step 7: Bro this is test_evaluate run only. We will not save any models here. So Chillax")
    else:
...
if __name__ == '__main__':
    ...
    report_dict = main(args)
    if (args.evaluate_test=="True"):
        logger.info(f'We are done bro.. test accuracy is {report_dict}')
    else:
