...
class Training():
    ...
    def train_model(self):
        ...
        if len(self.valid_history['ckpt_name']) > 0:
            ...
            print("nevertheless let us save last epoch's weights")
            self.save_checkpoint(epoch, la[-1], avg_epoch_train_loss)
...
def parse_inputs():
    ...
    parser.add_argument('--evaluate_test',
                        type=str,
                        default="False",
                        help="If set to True it will only run evaluate_test loop. Default: False",
                        )

    args = parser.parse_args()
    if args.evaluate_test == "True":
        logger.info(f'Evaluate test flag is set to: {args.evaluate_test}')
        logger.info("This is evaluate test run only. No Model Training will be performed")
    else:
...
def create_data_loaders(args, data_path, batch_size, image_augment):
    '''
...
def process_extract():
    ...
    logger.info(f'contens of model_path are: {os.listdir(model_path)}')
...
def create_model(args):
    ...
    device = 'cuda' if torch.cuda.is_available() else 'cpu'
    logger.info(f'chkpt_file_path is {chkpt_file_path}')
    logger.info(f'contents of mnodel_dir are {os.listdir(model_dir)}')

    arch = args.arch
    do = args.do
    num_class = args.num_class
    pre_weight_freeze = args.pre_weight_freeze
    num_cls_blocks = args.num_cls_blocks

    model = net(arch, do, num_class, pre_weight_freeze, num_cls_blocks)
    model.to(device)

    checkpoint = torch.load(chkpt_file_path, map_location=torch.device(device))
...
def main(args):
    ...
    train_dl, valid_dl, test_dl, train_ds, valid_ds, test_ds = create_data_loaders(args,
                                                                                   data_path,
                                                                                   batch_size,
                                                                                   args.image_augment)
    ...
    if args.evaluate_test=="True":
        logger.info("STEP 2: Loading Model checkpoint weights for test evaluation")
    ...
    if args.evaluate_test == "True":
        logger.info("STEP 5: Hey Bro!! evaluate_test flag is set to True so we will be doing test evaluation only")
        df, avg_loss, avg_test_accuracy = train_obj.get_accuracy(valid=False, train=False)
        report_dict = {
            "metrics": {
                "accuracy": {
                    "value": avg_test_accuracy,
                    },
                },
            }
        logger.info("Let us save the accuracy metrics in json format")
        output_data = "/opt/ml/processing/output/test"
        eval_metrics_path = f'{output_data}/metrics'
        os.makedirs(eval_metrics_path, exist_ok=True)
        evaluation_path = f'{eval_metrics_path}/evaluation.json'
        with open(evaluation_path, "w") as f:
            f.write(json.dumps(report_dict))
        logging.info(f'Listing contents of {eval_metrics_path} dir: {os.listdir(eval_metrics_path)}')
        logging.info(f'Eval report contents: {report_dict}')
...
if __name__ == '__main__':
    ...
    if args.evaluate_test == "False":
        ...
        logger.info("Bro.. This was test evaluate loop only. All Good. Exiting the script now")
    logger.info("Have a wonderful day ahead to all good souls")